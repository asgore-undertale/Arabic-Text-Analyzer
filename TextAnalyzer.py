from Un_Freeze_Arabic import Un_Freeze
...
def Analyze(text):
    ...
    while True:
        ...
        print(f'({Samples[k]}).count =', count, ',  Text length:', len(text))
        # t += f'({Samples[k]}).count = {count}, Text length: {len(text)}\n' #
    
    print('\nNew text length\t\t=', len(text), '\nNew text:\n', text)
    # t += f'\nNew text length\t\t= {len(text)} \nNew text:\n {text}\n' #
    # open('t.txt', 'w', encoding='utf-8').write(t) #
    return text

text = Analyze(open(input('Enter file. '), 'r', encoding='utf-8').read())
input(text)